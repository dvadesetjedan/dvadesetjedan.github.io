...
function assertUnique(entries, label) {
  ...
  for (const entry of entries) {
    ...
    if (seen.has(entry.slug))
      failures.push(`${label}: dupliciran slug ${entry.slug}.`)
    seen.add(entry.slug)
  }
}
...
function validCommunityProjectHref(value) {
  if (!value) return false
  if (value.startsWith("npub"))
    return /^npub1[023456789acdefghjklmnpqrstuvwxyz]+$/i.test(value)
...
const episodes = extractObjects(
  ...
  chapters: [
    ...objectSource.matchAll(/time:\s*"([^"]+)"\s*,\s*title:\s*"([^"]+)"/g),
  ].map((match) => ({
    time: match[1],
    title: match[2],
  })),
  links: [
    ...objectSource.matchAll(/label:\s*"([^"]+)"\s*,\s*href:\s*"([^"]+)"/g),
  ].map((match) => ({
    label: match[1],
    href: match[2],
  })),
...
const cities = extractObjects(
  readSource("src/data/cities.ts"),
  "export const cities",
).map((objectSource, index) => ({
  label: `Grad ${index + 1}`,
  slug: readString(objectSource, "slug"),
...
const communityProjects = extractObjects(
  readSource("src/data/communityProjects.ts"),
  "export const communityProjects",
).map((objectSource, index) => ({
  label: `Projekt iz zajednice ${index + 1}`,
  slug: readString(objectSource, "slug"),
  title: readString(objectSource, "title"),
  summary: readString(objectSource, "summary"),
  description: readStringArray(objectSource, "description"),
  category: readString(objectSource, "category"),
  secondaryCategories: readStringArray(objectSource, "secondaryCategories"),
  status: readString(objectSource, "status"),
  relationship: readString(objectSource, "relationship"),
  tags: readStringArray(objectSource, "tags"),
  topics: readStringArray(objectSource, "topics"),
  featured: readBoolean(objectSource, "featured") ?? false,
  consentConfirmed: readBoolean(objectSource, "consentConfirmed") ?? false,
  donationNote: readString(objectSource, "donationNote"),
  contactNote: readString(objectSource, "contactNote"),
  links: [
    ...objectSource.matchAll(
      /label:\s*"([^"]+)"\s*,\s*href:\s*"([^"]+)"\s*,\s*type:\s*"([^"]+)"\s*,\s*public:\s*(true|false)/g,
    ),
  ].map((match) => ({
    label: match[1],
    href: match[2],
...
const onboarding = extractObjects(
  readSource("src/data/onboarding.ts"),
  ...
  recommendedArticleSlugs: readStringArray(
    objectSource,
    "recommendedArticleSlugs",
  ),
  recommendedEpisodeSlugs: readStringArray(
    objectSource,
    "recommendedEpisodeSlugs",
  ),
}))

const legacyRedirects = extractObjects(
...
assertUnique(communityProjects, "Projekti iz zajednice")
...
const publishedCommunityProjects = communityProjects.filter(
  (entry) => entry.status !== "draft" && entry.consentConfirmed,
)
const knownRoutes = new Set([
  ...
  ...publishedCommunityProjects.map((entry) => `/iz-zajednice/${entry.slug}/`),
])

const validCommunityProjectCategories = new Set([
  "education",
  "translation",
  "event",
  "video",
...
const validCommunityProjectStatuses = new Set([
  "active",
  "planned",
  "archive",
  "draft",
])
const validCommunityProjectRelationships = new Set([
  "member-project",
  "community-initiative",
  "regional-partner",
])
const validCommunityProjectLinkTypes = new Set([
  "website",
  "youtube",
  "pdf",
  "github",
...
for (const article of articles) {
  ...
  if (!validDate(article.date))
    failures.push(`${article.label}: date nije validan.`)
  if (!Array.isArray(article.tags))
    failures.push(`${article.label}: tags mora biti array.`)
  if (!validUrl(article.originalUrl))
    failures.push(`${article.label}: originalUrl nije validan.`)
  if (!validUrl(article.image))
    failures.push(`${article.label}: image nije validan URL/path.`)
  for (const slug of article.recommendedNextSlugs) {
    if (!articleSlugs.has(slug))
      failures.push(
        `${article.label}: recommendedNextSlugs ne postoji: ${slug}.`,
      )
  }
}
...
for (const episode of episodes) {
  if (!episode.slug || !episode.title)
    failures.push(`${episode.label}: slug i title su obavezni.`)
  if (
    episode.youtubeUrl &&
    !/^https:\/\/(www\.)?youtube\.com\//.test(episode.youtubeUrl)
  ) {
  ...
  if (
    episode.needsShownotes !== undefined &&
    typeof episode.needsShownotes !== "boolean"
  ) {
  ...
  for (const chapter of episode.chapters) {
    if (!/^\d{1,2}:\d{2}(?::\d{2})?$/.test(chapter.time)) {
      failures.push(
        `${episode.label}: chapter time nije validan: ${chapter.time}.`,
      )
    }
  }
  for (const link of episode.links) {
    if (!link.label || !validUrl(link.href))
      failures.push(`${episode.label}: link nije validan.`)
  }
  for (const term of episode.terms) {
    if (!term.term || !term.explanation)
      failures.push(`${episode.label}: terms trebaju term i explanation.`)
  }
  for (const slug of episode.relatedArticleSlugs) {
    if (!articleSlugs.has(slug))
      failures.push(
        `${episode.label}: relatedArticleSlugs ne postoji: ${slug}.`,
      )
  }
  for (const slug of episode.relatedEventSlugs) {
    if (!eventSlugs.has(slug))
      failures.push(`${episode.label}: relatedEventSlugs ne postoji: ${slug}.`)
  }
}
...
for (const city of cities) {
  ...
  if (city.status === "emerging" && city.eventSlugs.length) {
    failures.push(
      `${city.label}: emerging grad ne smije imati eventSlugs bez javno potvrđenog događaja.`,
    )
  }
  for (const slug of city.eventSlugs) {
    if (!eventSlugs.has(slug))
      failures.push(`${city.label}: eventSlugs ne postoji: ${slug}.`)
  }
}

for (const project of communityProjects) {
  if (!project.slug || !project.title || !project.summary) {
    failures.push(`${project.label}: slug, title i summary su obavezni.`)
  }
  if (!project.description.length)
    failures.push(`${project.label}: description array je obavezan.`)
  if (!validCommunityProjectCategories.has(project.category)) {
    failures.push(`${project.label}: category nije validan.`)
  }
  for (const category of project.secondaryCategories) {
    if (!validCommunityProjectCategories.has(category)) {
      failures.push(
        `${project.label}: secondaryCategories sadrži nepoznatu kategoriju ${category}.`,
      )
    }
  }
  if (!validCommunityProjectStatuses.has(project.status)) {
    failures.push(`${project.label}: status nije validan.`)
  }
  if (!validCommunityProjectRelationships.has(project.relationship)) {
    failures.push(`${project.label}: relationship nije validan.`)
  }
  if (!project.tags.length)
    failures.push(`${project.label}: tags array je obavezan.`)
  if (project.topics.some((topic) => !topic.trim())) {
    failures.push(`${project.label}: topics ne smije imati prazne vrijednosti.`)
  }
  if (
    project.featured &&
    (project.status === "draft" || !project.consentConfirmed)
  ) {
    failures.push(
      `${project.label}: featured projekt mora biti javno objavljiv.`,
    )
  }
  for (const link of project.links) {
    if (!link.label || typeof link.public !== "boolean") {
      failures.push(
        `${project.label}: svaki link treba label i public boolean.`,
      )
    }
    if (!validCommunityProjectLinkTypes.has(link.type)) {
      failures.push(`${project.label}: link type nije validan: ${link.type}.`)
    }
    if (!validCommunityProjectHref(link.href)) {
      failures.push(`${project.label}: link href nije validan: ${link.href}.`)
    }
    if (
      ...
      failures.push(`${project.label}: internal link ne postoji: ${link.href}.`)
    }
    if (
      link.type === "telegram" &&
      /^https?:\/\/t\.me\/[^+]/.test(link.href) &&
      !project.contactNote
    ) {
      failures.push(
        `${project.label}: osobni Telegram link treba potvrdu u contactNote.`,
      )
    }
  }
  if (
    project.links.some((link) => link.public && link.type === "donation") &&
    !project.donationNote
  ) {
    failures.push(`${project.label}: javni donation link treba donationNote.`)
  }
}

if (onboarding.length !== 21)
  failures.push(
    `Onboarding mora imati točno 21 korak, trenutno ima ${onboarding.length}.`,
  )
for (const step of onboarding) {
  ...
  if (
    step.safetyLevel &&
    !["normal", "important", "critical"].includes(step.safetyLevel)
  ) {
  ...
  for (const slug of step.recommendedArticleSlugs) {
    if (!articleSlugs.has(slug))
      failures.push(
        `${step.label}: recommendedArticleSlugs ne postoji: ${slug}.`,
      )
  }
  for (const slug of step.recommendedEpisodeSlugs) {
    if (!episodeSlugs.has(slug))
      failures.push(
        `${step.label}: recommendedEpisodeSlugs ne postoji: ${slug}.`,
      )
  }
}
...
for (const redirect of legacyRedirects) {
  if (!redirect.from || !redirect.to || !redirect.reason)
    failures.push(`${redirect.label}: from, to i reason su obavezni.`)
  if (redirect.from?.includes("#") || redirect.to?.includes("#"))
    failures.push(`${redirect.label}: hash rute nisu dozvoljene.`)
  if (!redirect.from?.startsWith("/") || !redirect.from?.endsWith("/"))
    failures.push(`${redirect.label}: from mora početi i završiti s /.`)
  if (!redirect.to?.startsWith("/") || !redirect.to?.endsWith("/"))
    failures.push(`${redirect.label}: to mora početi i završiti s /.`)
  if (redirect.from === redirect.to)
    failures.push(`${redirect.label}: from i to ne smiju biti isti.`)
  if (redirectFromValues.has(redirect.from))
    failures.push(`${redirect.label}: dupliciran from ${redirect.from}.`)
  redirectFromValues.add(redirect.from)
  if (redirect.to && !knownRoutes.has(redirect.to))
    failures.push(`${redirect.label}: target route ne postoji: ${redirect.to}.`)
  if (legacyRedirects.some((entry) => entry.from === redirect.to))
    failures.push(
      `${redirect.label}: target ne smije biti legacy route: ${redirect.to}.`,
    )
}
